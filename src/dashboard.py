...
# dashboard.py — Streamlit SOC console for live pipeline monitoring,
# report history, and PDF/JSON downloads.
import os
import sys
import json
import glob
...
from utils.secure_config import get_missing_keys, save_env, load_env

# Load environment (project root .env is loaded as fallback)
load_env()
...
def inject_custom_css(T):
    ...
    .tlp-banner * { color: #22d3ee !important; }
    ...
    .status-online { background: #10b98122; color: #10b981 !important; border: 1px solid #10b98155; }
    .status-scanning { background: #f59e0b22; color: #f59e0b !important; border: 1px solid #f59e0b55; }
    .status-completed { background: #3b82f622; color: #3b82f6 !important; border: 1px solid #3b82f655; }
    .progress-track {
        background: #243248;
        border-radius: 100px;
        height: 7px;
    ...
    .stButton > button {
        ...
        color: #e6edf7 !important;
        border: 1px solid #243248 !important;
...
def render_notice(T, text, kind="info"):
    colors = {"info": "#22d3ee", "success": "#10b981", "warning": "#f59e0b", "error": "#ef4444"}
    icons = {"info": "ℹ️", "success": "✅", "warning": "⚠️", "error": "⛔"}
    c = colors.get(kind, "#22d3ee")
    st.markdown(f"""
...
def render_hero_header(T, status_text, status_class):
    ...
    <div class="hero-wrap">
        ...
        <div class="hero-subtitle" style="font-size:0.8rem; color:#7d8da3; text-transform:uppercase;">AI-POWERED THREAT INTELLIGENCE &amp; TRIAGE</div>
        <div class="hero-meta">
...
def render_executive_dashboard(T, agents_state):
    ...
    <div style="margin-bottom:1.4rem;">
        <div style="display:flex; justify-content:space-between; font-size:0.75rem; color:#7d8da3; font-family:'JetBrains Mono',monospace;">
    ...
    <div style="display:grid; grid-template-columns:repeat(4,1fr); gap:1rem; margin-bottom:0.5rem;">
        <div class="metric-card">
            <div class="metric-label" style="color:#7d8da3;">Total CVEs</div>
            <div class="metric-value">{total_cves}</div>
            <div style="font-size:0.7rem; color:#7d8da3;">Identified vulnerabilities</div>
        </div>
        <div class="metric-card">
            <div class="metric-label" style="color:#7d8da3;">Critical</div>
            <div class="metric-value" style="color:#ef4444 !important;">{critical_count}</div>
            <div style="font-size:0.7rem; color:#7d8da3;">Immediate action required</div>
        </div>
        <div class="metric-card">
            <div class="metric-label" style="color:#7d8da3;">High</div>
            <div class="metric-value" style="color:#f59e0b !important;">{high_count}</div>
            <div style="font-size:0.7rem; color:#7d8da3;">High priority</div>
        </div>
        <div class="metric-card">
            <div class="metric-label" style="color:#7d8da3;">Medium</div>
            <div class="metric-value" style="color:#22d3ee !important;">{medium_count}</div>
            <div style="font-size:0.7rem; color:#7d8da3;">Standard priority</div>
        </div>
    </div>
    """
...
def generate_agent_rings_html(T, agents_data):
    ...
    for agent in agents_data:
        ...
        color = '#10b981' if pct == 100 else ('#22d3ee' if pct > 0 else '#243248')
        html += f"""
        <div style="display:flex; flex-direction:column; align-items:center;">
            <div style="position:relative; width:78px; height:78px; display:flex; justify-content:center; align-items:center;">
                <svg width="78" height="78" style="position:absolute; top:0; left:0; transform:rotate(-90deg);">
                    <circle cx="39" cy="39" r="32" fill="none" stroke="#243248" stroke-width="6" />
                    <circle cx="39" cy="39" r="32" fill="none" stroke="{color}" stroke-width="6"
                            stroke-dasharray="{circumference}" stroke-dashoffset="{offset}"
                            style="transition: stroke-dashoffset 0.5s ease;" />
                </svg>
                <span style="font-size:1.05rem; font-weight:700; color:#ffffff; font-family:'JetBrains Mono',monospace;">{pct}%</span>
            </div>
            <span style="margin-top:0.5rem; font-size:0.7rem; font-weight:700; color:#7d8da3; text-transform:uppercase; letter-spacing:0.05em;">{agent['name']}</span>
        </div>
        """
    html += '</div>'
    return html
...
def render_console(T, log_lines, height=300):
    ...
    html = f"""
    <div style="background:#000000; border:1px solid #243248;
                border-radius:10px; overflow:hidden; margin-top:0.5rem;">
        <div style="background:#091018; padding:0.45rem 0.9rem;
                    display:flex; align-items:center; gap:0.4rem;
                    border-bottom:1px solid #243248;
                    font-family:'JetBrains Mono',monospace; font-size:0.7rem; color:#7d8da3;">
        ...
        <div id="smart-console"
             style="font-family:'JetBrains Mono',monospace; font-size:0.8rem;
                    ...
                    line-height:1.65; background:#000000;">
            {rows_html}
        </div>
    </div>
    <script>
        ...
        <div style="background:#000000; border:1px solid #243248;
            ...
            <div id="raw-console"
                 style="font-family:'JetBrains Mono',monospace; font-size:0.75rem;
                        color:#22d3ee; padding:0.9rem 1rem; height:260px;
                        overflow-y:auto; white-space:pre-wrap; line-height:1.6;
                        word-break:break-all; background:#000000;">
                {safe_raw}
            </div>
        </div>
        <script>
...
def run_agent_with_console(ring_placeholder, console_placeholder, T, agents_data, current_index, script_name):
    try:
        # Run the agent with project root as working directory
...
def display_statistics(T):
    ...
    st.markdown(f"<h3 style='font-weight:700; color:#ffffff;'>📊 Detailed Vulnerability Findings</h3>", unsafe_allow_html=True)
    pub_data, _ = load_latest_json(os.path.join(DATA_DIR, "publisher_agent_result"))
...
def display_console_tools(T):
    st.markdown("<hr>", unsafe_allow_html=True)
    st.markdown(f"<h3 style='font-weight:700; color:#ffffff;'>🖥️ Console Tools</h3>", unsafe_allow_html=True)
    ...
    pub_data, pub_path = load_latest_json(os.path.join(DATA_DIR, "publisher_agent_result"))
    with c2:
        if pub_data:
...
def display_report_history(T):
    st.markdown("<hr>", unsafe_allow_html=True)
    st.markdown(f"<h3 style='font-weight:700; color:#ffffff;'>📂 Report History</h3>", unsafe_allow_html=True)
    report_dir = os.path.join(DATA_DIR, "Reports")
...
def main():
    ...
    # --- API Key UI (show if any keys are missing) ---
    missing = get_missing_keys()
    if missing:
        st.warning("🔑 Some API keys are missing. Please enter them below to enable the pipeline.")
        with st.form("api_keys_form"):
            new_env = {}
            for key, url in missing.items():
                st.markdown(f"**{key}** ([get it here]({url}))")
                val = st.text_input(f"Enter {key}", type="password", key=f"key_{key}")
                new_env[key] = val
            ...
            if submitted:
                ...
                for k, v in new_env.items():
                    if v:
                        current[k] = v
                save_env(current)
                st.success("✅ Keys saved! Please reload the page.")
                st.rerun()
